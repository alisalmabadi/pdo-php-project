           ...
           $name = prevent($_POST["name"]);
            ...
            $edit_id=checkid($_POST["edit_id"]);
              ...
              if($_POST["password"]!=""){
                ...
                $resultamc->bindValue(16,$edit_id);
                ...
                $resultama->bindValue(15,$edit_id);
                             ...
                             ویرایش نام کاربری
									...
									<form action="" class="form-horizontal" method="post">
                                        ...
                                        <div class="form">
                                                             ...
                                                             <div class="panel-body">
    <?php
    $sms="SELECT `username`,`id` FROM `$tbl_user` WHERE `id`=:id";
     $resss=$connect->prepare($sms);
     $resss->bindParam(":id",$eid);
 $resss->execute();
     ...
     ?>
      <input type="hidden" name="edit_ida" value="<?=$rowsa["id"];?>">
                                                                           
											   <label class="col-sm-2 control-label">نام کاربری</label><br>
                                        <div class="col-sm-10">
                                            <input name="username" type="text" value="<?=$rowsa["username"];?>" class="form-control"><br><br>
        ...
        <?php
          ...
          ?>
                                            ...
                                            <button type="submit" class="btn btn-warning btn-block" name="edit_username">ویرایش نام کاربری</button>
                    ...
                    <div class="col-lg-12">
                        <section class="panel">
                                ...
                                <section class="panel">
                                    <header class="panel-heading">
                             ویرایش گروه و سمت کاربری
                                 
                                    </header>
   
									<form action="" class="form-horizontal" method="post">
									
									      <input type="hidden" class="edit_user" id="edit_user" name="edit_id" class="edit_id_user" value="<?=$row["id"];?>">
     ...
     $sql2="SELECT * FROM `$tbl_group` WHERE `id`=:id";
      ...
      while($rows2=$res2->fetch(PDO::FETCH_ASSOC)){
          ?>
            <div class="btn-group">
                                    <button class="btn btn-success" type="button"> &nbsp; <?=$rows2['title'];?> </button>
                                    <button data-toggle="dropdown" class="btn btn-success dropdown-toggle" type="button"><span class="caret"></span></button>
                                    <ul role="menu" class="dropdown-menu">
                                       
                                        <li class="divider"></li>
                             <li class="delgs" value="<?=$rows55['group_id'];?>"><a href="#" onclick="Relaod_Page()">حذف</a></li>
                                    </ul>
                                </div>
            ...
            <?php
                ...
                /*
     $g_id=$row["group_id"];                                          
    $sms="SELECT * FROM `$tbl_group` WHERE `id`=:id";
    $resam=$connect->prepare($sms);
    $resam->bindParam(":id",$g_id);
    $resam->execute();
    while($table=$resam->fetch(PDO::FETCH_ASSOC)){
                                 */
    ?> 
...
<select class='form-control input-sm m-bot15 semat' name="semat" id="semat">                                                     
    ...
    <?php echo"سمت های فعلی:";?>
                <?php 
      $sqlsa="SELECT * FROM `$tbl_group_semat` WHERE `user_id`=:id";
      $res555=$connect->prepare($sqlsa);
      $res555->bindParam(":id",$eid);
      $res555->execute();
                                         
      while($rows555=$res555->fetch(PDO::FETCH_ASSOC)){
      $sid=$rows555["semat_id"];
     $sql23="SELECT * FROM `$tbl_semat` WHERE `id`=:id";
      $res23=$connect->prepare($sql23);
      $res23->bindParam(":id",$sid);
      $res23->execute();
      while($rows23=$res23->fetch(PDO::FETCH_ASSOC)){
          ?>
          
                                            
                                            
                                                 <div class="btn-group">
                                    <button class="btn btn-danger" type="button"> &nbsp; <?=$rows23['title'];?> </button>
                                    <button data-toggle="dropdown" class="btn btn-danger dropdown-toggle" type="button"><span class="caret"></span></button>
                                    <ul role="menu" class="dropdown-menu">
                                       
                                        <li class="divider"></li>
         <li class="delgs2" value="<?=$rows555['semat_id'];?>"><a href="#" onclick="Relaod_Page()">حذف</a></li>
                                    </ul>
                                </div>
	...
	<button type="submit" class="btn btn-primary btn-block" name="edit_gs">افزودن گروه و سمت کاربری</button>
