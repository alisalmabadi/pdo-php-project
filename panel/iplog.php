...
<head>
    ...
    <title>آخرین ورودی کاربران</title>
...
?>
        ...
        <section id="main-content">
            <section class="wrapper">
                    ...
                    <div class="col-lg-12">
                        <section class="panel">
                            ...
                            <table class="table table-striped border-top" id="sample_1">
								...
								$s="SELECT * FROM `$tbl_ip` WHERE `type`=2 ORDER BY `id` ASC";
								$rmsb=$connect->query($s);
								while($rowsha=$rmsb->fetch(PDO::FETCH_ASSOC)){
                                     $user_id=$rowsha["user_id"];
								?>
             ...
             $sqls="SELECT `name`,`lastname` FROM `$tbl_user` WHERE `id`=:id ORDER BY `id`";
                $natije=$connect->prepare($sqls);
                $natije->bindParam(":id",$user_id);
                $natije->execute();
                $rowsasd=$natije->fetch(PDO::FETCH_ASSOC);           
                      
                                    ?>
            <td><?=$rowsasd['name'];?>&nbsp;<?=$rowsasd['lastname'];?></td>
