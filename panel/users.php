...
?>
                ...
                <div class="row">
                    <div class="col-lg-12">
					  ...
					  $sqls="SELECT `title` FROM `$tbl_group` WHERE `id`=:id";
					  $natije=$connect->prepare($sqls);
					  $natije->bindParam(":id",$gid);
					  $natije->execute();
					  $satr=$natije->fetch(PDO::FETCH_ASSOC);
					  echo "اعضا گروه"."&nbsp".$satr['title'];
								?>
                            ...
                            <table class="table table-striped border-top" id="sample_1">
								...
								<center>
                                    ...
                                    <tr>
				                       ...
				                       <th class="hidden-phone">سمت ها در این گروه</th>
								...
								<?php
                $sqls="SELECT * FROM `$tbl_group_semat` WHERE `group_id`=:gid";
                $res=$connect->prepare($sqls);
                $res->bindParam(":gid",$gid);
                $res->execute();
              while($rowshams=$res->fetch(PDO::FETCH_ASSOC)){ 
               $user_id=$rowshams['user_id'];

                   
                   $s="SELECT * FROM `$tbl_user` WHERE `id`=:id ORDER BY `id` DESC";
								$rmsb=$connect->prepare($s);
								$rmsb->bindParam(":id",$user_id);
								$rmsb->execute();
     ...
     $res59->execute();
      ...
      while($semat=$res5->fetch(PDO::FETCH_ASSOC)){
               echo '<font style="border-radius:5px;color:white; border:1px solid black; padding: 2px; background:#219fae;">'.$semat['title'].'</font>'.'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<br><br>';
											 ...
											 $semat_id=$rowshams['semat_id'];
											 $sql="SELECT `title` FROM `$tbl_semat` WHERE `id`=:id";
											 $result=$connect->prepare($sql);
											 $result->bindParam(":id",$semat_id);
											 $result->execute();
							...
							?>
								...
								<a  href="?del_id=<?php echo $idha;?>"> <button type="button" class="btn btn-round btn-danger"><i class="icon-remove-sign"></i>حذف</button></a>
                           	    <a href="edit_users.php?edit_id=<?php echo $idha;?>"> <button type="button" class="btn btn-round btn-warning"><i class="icon-pencil"></i>ویرایش</button></a>
                                     <a  href="user_profile.php?edit_id=<?php echo $idha;?>"> <button type="button" class="btn btn-round btn-primary"><i style="padding:5px;" class="icon-camera"></i>پروفایل کاربری</button></a>
